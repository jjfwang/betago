...
import { rankChessMoves, recommendedChessMove } from "./eval.js";
...
function summarizeChessCandidates(game, legalMoves) {
  return rankChessMoves(game.state, legalMoves, "black", 12);
}
...
function buildPayload(game, legalMoves) {
  const recommendedMove = recommendedChessMove(game.state, legalMoves, "black");
  ...
  return {
    ...
    candidate_moves: summarizeChessCandidates(game, legalMoves),
...
function buildOpenAIRequest(game, payload) {
  return {
    ...
    instructions: [
      ...
      "Use legal_moves as the source of truth. candidate_moves are heuristically ranked hints generated by the local engine.",
      "Prefer tactically sound moves that avoid blunders, answer threats, improve development, and win material safely.",
